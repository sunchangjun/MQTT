...
public class TransactionController {
	...
	public String createOrder(String ids, String $key, String state) {
		...
		if (!ischecked) {
			...
			try {
				sb.append("http://api.tiand99.com/box/box_order_pay.action?error="+URLEncoder.encode(error,"utf-8")+"&state=" + state
						+ "&$key=" + key1);
		...
		for (int i = 0; i < _ids.size(); i++) {
			...
			int _num = saleNum;
			List<CommodityRoad> listroad = commodityRoadRepository._findAllByBoxAndBoxPortCateAndCommodity(box,boxPortCate,commodity);
		...
		if(isok) {
			...
			List<String> values = new ArrayList<String>();
			values.add(od.getOrderId().toString());
			values.add(state);
			String key1 = MyKeyCreater.createKey(values);
			System.out.println(JSON.toJSONString(orderRepository.findOne(od.getOrderId())));
			sb.append("http://api.tiand99.com/box/box_order_pay.action?order=" + od.getOrderId() + "&state=" + state
					+ "&$key=" + key1);
			return sb.toString();
		}
	...
	@ResponseBody
	public Result orderInfo(String orderId, String $key) {
		...
		if (order != null) {
			...
			values.add(json);
			String key12 = MyKeyCreater.createKey(values);
			Result ret = new Result();
			ret.set$key(key12);
			ret.setResult(json);
			ret.setSuccess(true);
