...
//	List<CommodityRoad> findByBoxIdAndCommodityId(Long commodityId,Long box_id);
	...
	@Query(value = "select a from CommodityRoad a where a.box=?1 and a.boxPort.boxPortCate=?2 and a.commodity=?3 and a.commodityNum>0 and a.isEnable=1")
	List<CommodityRoad> _findAllByBoxAndBoxPortCateAndCommodity(Box box, BoxPortCate boxPortCate, Commodity commodity);
