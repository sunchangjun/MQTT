...
(function () {
    DD.createModule({
        name: "m_group_menu",
        delayInit: true,
        templateUrl: BASEHTML + '/auth/group_menu/group_menu.html',
        data: {
            left_page: 1,
            left_searchName: '',
            left_total:0,
        ...
        methods: {
            ...
            updateBinds: function (params) {
                var me = this;
	            DD.request({
					url:DATAURL + '/back/auth/group/getMenu',
					params: params,
					reqType:'POST',
					type:'json',
					async:true,
					...
					successFunc:function(r){
						...
						var param = {							/*右侧权限*/
							title:me.data.right_searchName,
            ...
            updateRightPage: function (param) {
	            ...
	            DD.request({
					...
					successFunc:function(r){
						...
						me.data.menu_list.forEach(function (item) {			/*对比列表项是否绑定*/
		                	me.data.bind_list.forEach(function (i) {
			                    if (item.menuId == i.menuId) {
            ...
            */
            addBind: function (e, d, v) {
                ...
                for(var i=0;i<me.data.bind_list.length;i++){
                	var str = me.data.bind_list[i].menuId;
                	arr_ids.push(str);
                };
                me.data.ids = arr_ids.join(",");
                DD.request({
					url:DATAURL + '/back/auth/group/updMenu',
					params: {
						groupId:me.data.group_id,
						ids:me.data.ids,
					},
